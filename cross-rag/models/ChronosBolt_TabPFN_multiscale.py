...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, augment: str):
        ...
        if 'moe' in self.augment:
            # moe: multi-scale pooling of retrieved_y
            ...
            scales_env = os.getenv("TABPFN_SCALES", "4,8,16")
            self.scales = [int(x) for x in scales_env.split(",") if x.strip()]
            # linear in_features should match pooled length (prediction_length // s)
            self.scale_mlps = nn.ModuleDict()
            for s in self.scales:
                pooled_len = max(1, self.chronos_config.prediction_length // s)
                self.scale_mlps[str(s)] = nn.Linear(pooled_len, config.d_model)
    ...
    def forward(
        ...
        # Zeroshot can pass a single example; ensure batch dimension exists
        if context.dim() == 1:
            context = context.unsqueeze(0)
            if mask is not None and mask.dim() == 1:
                mask = mask.unsqueeze(0)
            if retrieved_seq is not None and retrieved_seq.dim() == 2:
                retrieved_seq = retrieved_seq.unsqueeze(0)
        ...
        if self.augment == 'moe':
            # Multi-scale tokens from retrieved_y, then self-attn with query
            multi_tokens = []
            for i in range(r_M):
                # original scale
                multi_tokens.append(self.encode_mlp(retrieved_y[:, i, :]))
                # pooled scales
                for s in self.scales:
                    pooled = F.avg_pool1d(retrieved_y[:, i, :].unsqueeze(1), kernel_size=s, stride=s)  # B,1,L'
                    pooled = pooled.squeeze(1)  # B,L'
                    token = self.scale_mlps[str(s)](pooled)
                    multi_tokens.append(token)
            multi_tokens = torch.stack(multi_tokens, dim=1)  # B, K*(1+len(scales)), d

            # append query
            tokens = torch.cat([multi_tokens, sequence_output], dim=1)  # B, M, d
            att_output, _ = self.mha(tokens, tokens, tokens)
            att_output = tokens + att_output
            att_output = att_output + self.dropout(self.ffn(att_output))
            sequence_output = sequence_output + att_output[:, -1:, :]  # take query slot
