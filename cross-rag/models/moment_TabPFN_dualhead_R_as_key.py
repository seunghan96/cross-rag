...
class MOMENTWithRetrieval(nn.Module):
    def __init__(self, config: Union[Namespace, dict], **kwargs: dict):
        ...
        self.self_mha = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=8, batch_first=True)
        self.cross_mha = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=8, batch_first=True)
    ...
    def forecast(
        ...
        **kwargs
        ...
        sequence_output = self.project_before_fusion(enc_out).unsqueeze(1)
        ...
        gate = self.mix_lambda_init 
        #fused = gate * cross_out + (1 - gate) * self_pool
        fused = gate * cross_out + (1 - gate) * self_pool
        #print('fused',fused.shape)
        sequence_output = fused.squeeze(1)  # B,1,d
        sequence_output = self.dropout(sequence_output)

        # print('enc_sequence_outputout',sequence_output.shape)
        # print('all_enc',all_enc.shape)

        # Step 5: skip connection
        # print('enc_ou1',enc_out.shape)
        enc_out = enc_out + self.project_after_fusion(sequence_output)            # B, 1, d_model
        # print('enc_out2',enc_out.shape)
        # print('self.project_after_fusion(sequence_output.squeeze(1))',self.project_after_fusion(sequence_output.squeeze(1)).shape)
