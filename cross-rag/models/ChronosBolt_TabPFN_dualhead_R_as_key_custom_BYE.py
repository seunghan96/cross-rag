...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, augment: str):
        ...
        input_seq_length = self.chronos_config.context_length
        ...
        if 'moe' in self.augment:
            ...
            print('self.chronos_config',self.chronos_config)
            print('config',config)
            self.encode_mlp_x = nn.Sequential(
                nn.Linear(input_seq_length, config.d_model),
                nn.ReLU(),
                nn.Linear(config.d_model, config.d_model),
            )
    ...
    def forward(
        ...
        if self.augment == 'moe':
            ...
            retrieved_y_enc = []
            for i in range(r_M):
                print(111,retrieved_x[:, i, :].shape)
                print(222,retrieved_y[:, i, :].shape)
