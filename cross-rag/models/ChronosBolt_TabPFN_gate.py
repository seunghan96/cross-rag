...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def forward(
        ...
        if self.augment == 'moe':
            # TabPFN_gate: concat, self-attend, then gated pooling over all tokens
            retrieved_y_enc = []
            for i in range(r_M):
            ...
            scores = torch.sigmoid(self.gate_layer(att_output))  # B, K+1,1
            alpha = F.softmax(scores, dim=1)  # B, K+1,1

            fused = torch.sum(alpha * att_output, dim=1, keepdim=True)  # B,1,d
            fused = self.dropout(fused)
            sequence_output = sequence_output + fused  # skip with query
