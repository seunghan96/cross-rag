...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, augment: str):
        ...
        if 'moe' in self.augment:
            # moe: content attn + rank-based bias fusion
    ...
    def forward(
        ...
        if self.augment == 'moe':
            # Rank/positional bias from distance ordering, fused with content attn
            retrieved_y_enc = []
            for i in range(r_M):
            ...
            # content cross-attn
            att_output, _ = self.mha(sequence_output, retrieved_y_enc, retrieved_y_enc)
            att_output = sequence_output + att_output
            att_output = att_output + self.dropout(self.ffn(att_output))

            # distance-based rank bias weights
            query_vec = sequence_output[:, 0, :]  # B,d
            dists = torch.cdist(query_vec.unsqueeze(1), retrieved_y_enc, p=2).squeeze(1)  # B,K
            ranks = torch.argsort(torch.argsort(dists, dim=1), dim=1).float()  # 0..K-1
            sharp = float(os.getenv("TABPFN_POSBIAS_SHARP", "1.0"))
            bias_w = torch.softmax(-ranks * sharp, dim=1)  # closer ranks get higher
            bias_vec = torch.sum(bias_w.unsqueeze(-1) * retrieved_y_enc, dim=1, keepdim=True)  # B,1,d

            sequence_output = att_output + bias_vec  # combine bias with content
