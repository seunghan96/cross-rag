...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, augment: str):
        ...
        if 'moe' in self.augment:
            ...
            self.self_mha = nn.MultiheadAttention(embed_dim=self.chronos_config.prediction_length, num_heads=8, batch_first=True)
            self.encode_mlp2 = nn.Sequential(
                nn.Linear(self.chronos_config.prediction_length, config.d_model),
                nn.ReLU(),
                nn.Linear(config.d_model, config.d_model),
            )
    ...
    def forward(
        ...
        if self.augment == 'moe':
            ...
            for i in range(r_M):
                ...
                #retrieved_y_enc.append(retrieved_y[:, i, :])
            ...
            self_out, _ = self.self_mha(retrieved_y, retrieved_y, retrieved_y)
            self_out = retrieved_y_enc + self.encode_mlp2(self_out)
