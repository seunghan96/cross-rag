...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def forward(
        ...
        if self.augment == 'moe':
            # TabPFN_concat: concat retrieved encodings + query token, self-attend, then use query slot
            retrieved_y_enc = []
            for i in range(r_M):
                retrieved_y_enc.append(self.encode_mlp(retrieved_y[:, i, :]))
            retrieved_y_enc = torch.stack(retrieved_y_enc, dim=1)  # B, K, d

            # concat retrieved tokens followed by query token
            ...
            # take the query position (last token) and skip-connect with original query
            query_out = att_output[:, -1:, :]  # B,1,d
            sequence_output = sequence_output + query_out
