...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, augment: str):
        ...
        if 'moe' in self.augment:
            ...
            # adaptive temperature head (query -> tau)
    ...
    def forward(
        ...
        if self.augment == 'moe':
            # Adaptive temperature per-query for distance kernel fusion
            retrieved_y_enc = []
            for i in range(r_M):
            ...
            # predict tau per query
            tau = torch.exp(self.temp_head(query_vec)) + 1e-6  # B,1
            ...
            # combine with cross-attn for refinement
