...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def forward(
        ...
        if self.augment == 'moe':
            # Two-stage: coarse via distance kernel on retrieved_y, then cross-attn refinement
            retrieved_y_enc = []
            for i in range(r_M):
                retrieved_y_enc.append(self.encode_mlp(retrieved_y[:, i, :]))
            retrieved_y_enc = torch.stack(retrieved_y_enc, dim=1)  # B, K, d

            # Coarse weights by distance to query embedding
            query_vec = sequence_output[:, 0, :]  # B,d
            dists = torch.cdist(query_vec.unsqueeze(1), retrieved_y_enc, p=2).squeeze(1)  # B,K
            coarse_w = torch.softmax(-dists, dim=1)  # B,K
            coarse_y = torch.sum(coarse_w.unsqueeze(-1) * retrieved_y, dim=1, keepdim=True)  # B,1,L

            # refine: query attends to retrieved_y_enc
            ...
            # fuse coarse signal encoded
            coarse_enc = self.encode_mlp(coarse_y.squeeze(1))
            fused = att_output + coarse_enc.unsqueeze(1)
