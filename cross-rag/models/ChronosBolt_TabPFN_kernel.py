...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, augment: str):
        ...
        if 'moe' in self.augment:
            ...
            # learnable temperature (log space) for kernel attention
            self.kernel_log_tau = nn.Parameter(torch.tensor(0.0))
    ...
    def forward(
        ...
        if self.augment == 'moe':
            # Kernel-attn hybrid: content attn + distance-kernel weighting
            retrieved_y_enc = []
            for i in range(r_M):
                retrieved_y_enc.append(self.encode_mlp(retrieved_y[:, i, :]))
            retrieved_y_enc = torch.stack(retrieved_y_enc, dim=1)  # B, K, d

            # Content self-attention over retrieved + query
            all_enc = torch.cat([retrieved_y_enc, sequence_output], dim=1)  # B, K+1, d
            content_out, _ = self.mha(all_enc, all_enc, all_enc)
            content_out = all_enc + content_out
            content_out = content_out + self.dropout(self.ffn(content_out))
            content_query = content_out[:, -1:, :]  # B,1,d

            # Distance kernel over retrieved encodings vs query encoding
            query_vec = sequence_output[:, 0, :]  # B,d
            dists = torch.cdist(query_vec.unsqueeze(1), retrieved_y_enc, p=2).squeeze(1)  # B,K
            # learnable temperature (registered in __init__)
            tau = torch.exp(self.kernel_log_tau) + 1e-6
            kernel_w = torch.softmax(-dists / tau, dim=1)  # B,K
            kernel_vec = torch.sum(kernel_w.unsqueeze(-1) * retrieved_y_enc, dim=1, keepdim=True)  # B,1,d

            fused = content_query + kernel_vec  # combine
            sequence_output = sequence_output + self.dropout(fused)  # skip with query
