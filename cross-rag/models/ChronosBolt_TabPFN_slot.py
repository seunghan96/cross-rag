...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def forward(
        ...
        if self.augment == 'moe':
            # Slot selection: score each retrieved token, fuse selected, then attend with query
            retrieved_y_enc = []
            for i in range(r_M):
            ...
            # scores for slots
            scores = torch.sigmoid(self.gate_layer(retrieved_y_enc)).squeeze(-1)  # B,K
            alpha = torch.softmax(scores, dim=1)  # B,K
            fused_slot = torch.sum(alpha.unsqueeze(-1) * retrieved_y_enc, dim=1, keepdim=True)  # B,1,d

            # attend over [fused_slot, query]
            tokens = torch.cat([fused_slot, sequence_output], dim=1)  # B,2,d
            ...
            # take query position (index 1)
            sequence_output = sequence_output + att_output[:, 1:2, :]
