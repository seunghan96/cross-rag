...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, augment: str):
        ...
        if 'moe' in self.augment:
            # moe: prototype compression then cross-attn
            ...
            # learnable prototypes (P x d_model)
            self.num_prototypes = int(os.getenv("TABPFN_NUM_PROTOTYPES", "4"))
            self.prototype_tokens = nn.Parameter(torch.randn(self.num_prototypes, config.d_model) * 0.02)
    ...
    def forward(
        ...
        if self.augment == 'moe':
            # Prototype compression of retrieved_y_enc, then query attends to prototypes
            retrieved_y_enc = []
            for i in range(r_M):
                retrieved_y_enc.append(self.encode_mlp(retrieved_y[:, i, :]))
            retrieved_y_enc = torch.stack(retrieved_y_enc, dim=1)  # B, K, d

            # compute attention weights to prototypes (content-based)
            # prototypes: (P,d) -> expand to (B,P,d)
            proto = self.prototype_tokens.unsqueeze(0).expand(batch_size, -1, -1)  # B,P,d
            # score_{b,p,k} = dot(proto_p, r_k)
            scores = torch.einsum('bpd,bkd->bpk', proto, retrieved_y_enc)  # B,P,K
            alpha = torch.softmax(scores, dim=-1)  # over K
            proto_tokens = torch.einsum('bpk,bkd->bpd', alpha, retrieved_y_enc)  # B,P,d

            # query attends to prototypes (KV=protos)
            att_output, _ = self.mha(sequence_output, proto_tokens, proto_tokens)
            att_output = sequence_output + att_output
            att_output = att_output + self.dropout(self.ffn(att_output))
            sequence_output = att_output  # B,1,d
