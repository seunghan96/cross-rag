...
class ChronosBoltModelForForecasting(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        self.input_seq_length = config.chronos_config["context_length"]
        self.output_seq_length = config.chronos_config["prediction_length"]
...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, augment: str):
        ...
        if 'gate' in self.augment:
            ...
            self.gate_layer = nn.Sequential(
                nn.Linear(self.output_seq_length * 2 + self.input_seq_length + 12, config.d_model),
                nn.ReLU(),
                nn.Linear(config.d_model, config.d_model),
            )
            self.gate_linear1 = nn.Linear(self.output_seq_length * 2 + self.input_seq_length + 12, config.d_model)
    ...
    def _pad_or_trim_seq(self, tensor: torch.Tensor, target_len: int) -> torch.Tensor:
        """Pad with zeros or trim from the end to match target length."""
        if tensor is None:
            return tensor
        ...
        if cur_len == target_len:
            return tensor
        if cur_len < target_len:
            pad = tensor.new_zeros(*tensor.shape[:-1], target_len - cur_len)
    ...
    def forward(
        ...
        expected_len = self.input_seq_length + self.output_seq_length
        if retrieved_seq is not None and retrieved_seq.shape[-1] != expected_len:
            retrieved_seq = self._pad_or_trim_seq(retrieved_seq, expected_len)
        expected_len = self.input_seq_length + self.output_seq_length
        if retrieved_seq is not None and retrieved_seq.shape[-1] != expected_len:
            retrieved_seq = self._pad_or_trim_seq(retrieved_seq, expected_len)
