...
class ChronosBoltModelForForecasting(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        # dynamic horizons
        self.input_seq_length = self.chronos_config.context_length
        self.output_seq_length = self.chronos_config.prediction_length
...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config, augment: str):
        ...
        if 'gate' in self.augment:
            ...
            self.gate_layer = nn.Sequential(
                nn.Linear(self.chronos_config.prediction_length * 2 + self.input_seq_length + 12, config.d_model),
                nn.ReLU(),
                nn.Linear(config.d_model, config.d_model),
            )
            self.gate_linear1 = nn.Linear(self.chronos_config.prediction_length * 2 + self.input_seq_length + 12, config.d_model)
            self.gate_linear2 = nn.Linear(config.d_model, 1)

        if 'moe' in self.augment:
            # dual-head: query->retrieved cross attn, retrieved self-attn summary, mix via gate
            print('self.chronos_config.context_length',self.chronos_config.context_length)
            self.encode_mlp_x = nn.Sequential(
                nn.Linear(self.input_seq_length, config.d_model),
                nn.ReLU(),
                nn.Linear(config.d_model, config.d_model),
            )

            self.encode_mlp_y = nn.Sequential(
                nn.Linear(self.output_seq_length, config.d_model),
                nn.ReLU(),
                nn.Linear(config.d_model, config.d_model),
            )
    ...
    def _pad_or_trim_seq(self, tensor: torch.Tensor, target_len: int) -> torch.Tensor:
        """Pad with zeros or trim from the start to match target length."""
        cur_len = tensor.shape[-1]
        if cur_len == target_len:
            return tensor
        if cur_len < target_len:
            pad_shape = (*tensor.shape[:-1], target_len - cur_len)
            pad = tensor.new_zeros(pad_shape)
            return torch.cat([tensor, pad], dim=-1)
        # trim from the end to keep the most recent portion
    ...
    def forward(
        ...
        expected_retrieve_len = self.input_seq_length + self.output_seq_length
        if retrieved_seq is not None and retrieved_seq.shape[-1] != expected_retrieve_len:
            retrieved_seq = self._pad_or_trim_seq(retrieved_seq, expected_retrieve_len)
        ...
        if self.augment == 'moe':
            # Dual-head: cross-attn (query->retrieved) and self-attn summary (retrieved->retrieved)
            retrieved_x_enc = []
            retrieved_y_enc = []
            for i in range(r_M):
                retrieved_x_enc.append(self.encode_mlp_x(retrieved_x[:, i, :]))
                retrieved_y_enc.append(self.encode_mlp_y(retrieved_y[:, i, :]))
            retrieved_x_enc = torch.stack(retrieved_x_enc, dim=1)  # B,K,d
            retrieved_y_enc = torch.stack(retrieved_y_enc, dim=1)  # B,K,d

            # cross branch
            cross_out, _ = self.cross_mha(sequence_output, retrieved_x_enc, retrieved_y_enc)
            cross_out = sequence_output + cross_out
            cross_out = cross_out + self.dropout(self.ffn_cross(cross_out))

            # self branch: retrieved self-attn then mean pool
            ...
            # mix with gate
            mix_in = torch.cat([cross_out, self_pool], dim=-1)  # B,1,2d
            gate = torch.sigmoid(self.mix_gate(mix_in))  # B,1,1
            # initialize around mix_lambda_init if provided
            gate = gate * 0 + self.mix_lambda_init + (gate - 0.5) * 0  # bias toward init but keep grad
            fused = gate * cross_out + (1 - gate) * self_pool

            sequence_output = fused  # B,1,d
