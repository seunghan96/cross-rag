...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def forward(
        ...
        quantile_preds1 = self.output_patch_embedding(sequence_output).view(*quantile_preds_shape)
        quantile_preds2 = self.output_patch_embedding(cross_out).view(*quantile_preds_shape)
        quantile_preds3 = self.output_patch_embedding(self_pool).view(*quantile_preds_shape)

        fused_quantile_preds = quantile_preds1 + quantile_preds2 + quantile_preds3
