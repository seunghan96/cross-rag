...
class ChronosBoltModelForForecastingWithRetrieval(T5PreTrainedModel):
    ...
    def forward(
        ...
        if self.augment == 'moe':
            ...
            # Step 2: build distance-based attention bias and run MHA
            # distances derived from retrieved_y_enc
            distances = torch.cdist(retrieved_y_enc, retrieved_y_enc, p=2)
            distances = torch.nan_to_num(distances, nan=0.0, posinf=0.0, neginf=0.0)
            dist_max = distances.max(dim=1, keepdim=True).values
            distances = distances / (dist_max + 1e-8)  # normalize per batch
            if temperature <= 0:
                raise ValueError("temperature must be > 0")
            # apply softmax with temperature after normalization
            similarity = torch.softmax(-distances / temperature, dim=1)  # B x r_M
            similarity = torch.cat([torch.ones_like(similarity[:, :1]), similarity], dim=1)  # B x (r_M + 1)
            attn_bias = torch.log(torch.clamp(similarity, min=1e-8))  # bias in log space
            attn_bias = attn_bias.unsqueeze(1).repeat(1, r_M + 1, 1)  # B x L x L (shared across queries)
            attn_bias = attn_bias.unsqueeze(1).repeat(1, self.mha.num_heads, 1, 1)  # B x H x L x L
            # enforce mean(attn_bias)=1 per (batch, head) to keep scale parity
            attn_bias_mean = attn_bias.mean(dim=(-2, -1), keepdim=True)
            attn_bias = attn_bias - attn_bias_mean + 1.0
            attn_bias = attn_bias.reshape(batch_size * self.mha.num_heads, r_M + 1, r_M + 1)

            att_output, attn_weights = self.mha(all_enc, all_enc, all_enc, attn_mask=attn_bias)
