...
datasets="${DATASETS:-"ETTh1 ETTh2 ETTm1 ETTm2 weather"}"  # 모든 데이터셋
...
total_combinations=$((${#lookback_lengths[@]} * ${#similarity_spaces[@]} * ${#top_ks[@]} * ${#backbones[@]} * ${#output_norms[@]} * ${#output_norm_modes[@]} * $num_dataset * ($num_dataset-1)))
            ...
            for backbone in "${backbones[@]}"; do
                ...
                for dataset in "${dataset_list[@]}"; do
                  ...
                  for retrieve_db in "${dataset_list[@]}"; do
                    ...
                    filename_base="zeroshot_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}_retrdb_${retrieve_db}"
                    filename="${filename_base}.txt"
                    model_id_zeroshot="${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_${retrieve_suffix}${output_norm_suffix}_retrdb_${retrieve_db}"
