#!/bin/bash
# -*- coding: utf-8 -*-
# Random K retrieval experiment script: Zero-shot with random sampling (no pretrain)
# Uses existing pretrained checkpoints and retrieve_Z_random.py / retrieve_X_random.py
# Variables:
        ...
        for top_k in "${top_ks[@]}"; do
            for backbone in "${backbones[@]}"; do
                for output_norm in "${output_norms[@]}"; do
                    ...
                    # Create model_id for pretrain checkpoint lookup (without -random)
                    # Add output_norm suffix (similarity space와 무관)
                    output_norm_suffix=""
                    if [[ "$output_norm" == true ]]; then
                        output_norm_suffix="_outputnorm_${output_norm_mode}"
                    fi
                    model_id_pretrain="pretrain_${backbone}_lb${lookback_length}_${original_similarity_space}_k${top_k}${output_norm_suffix}"
                
                # Pretrain checkpoint directory (use existing checkpoint, skip pretrain)
                pretrain_checkpoint_dir="./checkpoints/${model_id_pretrain}"
                pretrain_checkpoint_path="${pretrain_checkpoint_dir}/best.pth"
                
                # ==========================================
                # Step 1: Use existing pretrained checkpoint (skip pretrain)
                # ==========================================
                echo ""
                ...
                # Create filename for results (add _random suffix)
                filename_base="zeroshot_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}_random"
                ...
                for dataset in $datasets; do
                    ...
                    # Create model_id for zero-shot (add _random suffix)
                    model_id_zeroshot="${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_${retrieve_suffix}${output_norm_suffix}_random"
