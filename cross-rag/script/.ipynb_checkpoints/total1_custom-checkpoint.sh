#!/bin/bash
# -*- coding: utf-8 -*-
# Total experiment script (custom horizons): Pretrain + Zero-shot for all combinations
# Based on total1.sh, extended to sweep input/forecast horizons.
...
# Horizons (context / prediction) can be overridden via env vars
input_horizons=(${INPUT_HORIZONS:-512})
forecast_horizons=(${FORECAST_HORIZONS:-16})
...
for forecast_horizon in "${forecast_horizons[@]}"; do
    ...
    # Scope checkpoints/results by horizons to avoid overwrite
