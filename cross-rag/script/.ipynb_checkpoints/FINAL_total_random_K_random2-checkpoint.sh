#!/bin/bash
# -*- coding: utf-8 -*-
# Random2 K retrieval experiment script: Pretrain & Zero-shot with random sampling
# Both pretrain and zero-shot use random retrieval (retrieve_Z_random.py / retrieve_X_random.py)
# Variables:
# - lookback_length: [96, 256, 512, 1024]
# - Similarity Space: [Z-random, X-random] (random sampling version)
# - K: [1, 3, 5, 10]
...
#datasets="${DATASETS:-"electricity"}"  # 모든 데이터셋
...
similarity_spaces=("Z-random")
#similarity_spaces=("X-random")
        ...
        for top_k in "${top_ks[@]}"; do
            for backbone in "${backbones[@]}"; do
                for output_norm in "${output_norms[@]}"; do
                    ...
                    # For random sampling: use Z-random or X-random
                    use_random=false
                    ...
                    if [[ "$similarity_space_base" == "Z" ]]; then
                        ...
                        if [[ "$use_random" == true ]]; then
                            retrieve_suffix="X-${metric}_random_k${top_k}"
                        elif [[ "$use_norm" == true ]]; then
                    ...
                    # Create model_id for pretrain (with random2 suffix for both pretrain and zero-shot random)
                    # Add output_norm suffix (similarity space와 무관)
                    output_norm_suffix=""
                    if [[ "$output_norm" == true ]]; then
                        output_norm_suffix="_outputnorm_${output_norm_mode}"
                    fi
                    model_id_pretrain="pretrain_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}${output_norm_suffix}_random2"
                ...
                # ==========================================
                # Step 1: Pretrain with random retrieval
                # ==========================================
                echo ""
                echo ">>> Step 1: Starting Pretrain with random retrieval..."
                ...
                if [ "$checkpoint_found" = false ]; then
                    ...
                    if [ "$skip_pretrain" = true ]; then
                        ...
                        # Set data_path based on similarity space (with random)
                        if [[ "$space" == "Z" ]]; then
                            # Z-space: use default folder (for random, we use the same data path)
                ...
                # Create filename for results (add _random2 suffix)
                filename_base="zeroshot_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}_random2"
                ...
                for dataset in $datasets; do
                    ...
                    # Create model_id for zero-shot (add _random2 suffix)
                    model_id_zeroshot="${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_${retrieve_suffix}${output_norm_suffix}_random2"
