...
# Soft-weight Chronos selection (softYenc variant)
export CHRONOS_SOFT_WEIGHT=1
export CHRONOS_SOFT_WEIGHT_VARIANT=softYenc
...
#temperatures=(${TEMPERATURES:-0.01 0.1 1.0})
temperatures=(${TEMPERATURES:-2.0 5.0 10.0})
...
for lookback_length in "${lookback_lengths[@]}"; do
    for similarity_space in "${similarity_spaces[@]}"; do
        for top_k in "${top_ks[@]}"; do
            for backbone in "${backbones[@]}"; do
                for output_norm in "${output_norms[@]}"; do
                    ...
                    model_id_pretrain="pretrain_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}${output_norm_suffix}_softYenc_v2_temp${temperature}"
                ...
                # Create filename for results (soft-weight V2: pretrain + zero-shot) + temperature suffix
                filename_base="zeroshot_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}_softYenc_v2_temp${temperature}"
                ...
                for dataset in $datasets; do
                    ...
                    model_id_zeroshot="${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_${retrieve_suffix}${output_norm_suffix}_softYenc_v2_temp${temperature}"
        ...
        done                   # output_norm_mode
    done                       # output_norm
done                           # backbone
done                           # top_k
done                           # similarity_space
done                           # lookback_length
