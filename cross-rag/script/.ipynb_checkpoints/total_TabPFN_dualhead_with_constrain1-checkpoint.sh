...
export CHRONOS_TABPFN_VARIANT=dualhead
export WANDB_DISABLED="${WANDB_DISABLED:-true}"
export WANDB_MODE="${WANDB_MODE:-disabled}"

run_file_pretrain=pretrain_constrain.py
...
similarity_spaces=("X-cosine" "X-cosine-norm" "Z")
top_ks=(5 10 15 20)
backbones=("Chronos")
output_norms=(false)
output_norm_modes=("x")
# hyperparameters: dualhead mix lambda init, and p for constrained pretrain
dual_lambda_list=(${TABPFN_DUAL_LAMBDA_LIST:-0.5 0.7})
p_list=(0.3)

total_combinations=$((${#lookback_lengths[@]} * ${#similarity_spaces[@]} * ${#top_ks[@]} * ${#backbones[@]} * ${#output_norms[@]} * ${#output_norm_modes[@]} * ${#dual_lambda_list[@]} * ${#p_list[@]}))
                ...
                for output_norm in "${output_norms[@]}"; do
                    for output_norm_mode in "${output_norm_modes[@]}"; do
                        for dual_lambda in "${dual_lambda_list[@]}"; do
                            ...
                            model_id_pretrain="pretrain_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}${output_norm_suffix}_TabPFN_dualhead_l${dual_lambda}_pretrain_prob${p}"
...
if [ "$pretrain_skipped" = true ]; then
    ...
    continue
                            ...
                            filename_base="zeroshot_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}_TabPFN_dualhead_l${dual_lambda}_pretrain_prob${p}"
                            ...
                            for dataset in $datasets; do
                                ...
                                model_id_zeroshot="${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_${retrieve_suffix}${output_norm_suffix}_TabPFN_dualhead_l${dual_lambda}_pretrain_prob${p}"
