#!/bin/bash
# -*- coding: utf-8 -*-
# Total experiment script: Zero-shot with alpha (train set percentage) for all combinations
# Note: Pretraining is skipped, uses existing checkpoints
# Variables:
...
datasets="${DATASETS:-"exchange_rate ETTh1 ETTh2 ETTm1 ETTm2 weather electricity"}"  # 모든 데이터셋
...
output_norm_modes=("x")
# alpha: percentage of train set to use for retrieval (e.g., 50 means last 50% of train set)
alpha_list=(${ALPHA_LIST:-20 25})

# Counter for tracking progress
total_combinations=$((${#lookback_lengths[@]} * ${#similarity_spaces[@]} * ${#top_ks[@]} * ${#backbones[@]} * ${#output_norms[@]} * ${#output_norm_modes[@]} * ${#alpha_list[@]}))
    ...
    for similarity_space in "${similarity_spaces[@]}"; do
        for top_k in "${top_ks[@]}"; do
            for backbone in "${backbones[@]}"; do
                ...
                # ==========================================
                # Step 1: Check Pretrain Checkpoint (스킵, 기존 checkpoint 사용)
                # ==========================================
                echo ""
                echo ">>> Step 1: Checking Pretrain Checkpoint (skipping pretrain, using existing)..."
                ...
                if [ -f "$pretrain_checkpoint_path" ]; then
                    ...
                    echo ">>> Using existing checkpoint for zero-shot..."
                    ...
                    echo ">>> No pretrained checkpoint found. Skipping zero-shot for this combination."
                ...
                if [ "$checkpoint_found" = false ]; then
                    echo ">>> Pretrained checkpoint not found. Skipping zero-shot for this combination."
                    continue
                fi
                
                echo ">>> Using checkpoint: $pretrain_checkpoint_path"
                
                # Pretrain is skipped - use existing checkpoint only
                if [ 1 = 0 ]; then
                    ...
                    if [ "$skip_pretrain" = true ]; then
                        ...
                        if [[ "$space" == "Z" ]]; then
                            ...
                            # X-rev, Z-rev, etc.: use folder with -rev suffix
                            if [[ "$space" == "X-rev" ]]; then
                                # X-space reverse: use folder with metric and norm suffix + -rev
                                if [[ "$use_norm" == true ]]; then
                                ...
                                # Z-space reverse: use default folder + -rev
                                if [[ "$use_norm" == true ]]; then
                ...
                # End of skipped pretrain block (pretrain is not executed)
                ...
                filename_base="zeroshot_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}_alpha${alpha}"
                ...
                for dataset in $datasets; do
                    ...
                    model_id_zeroshot="${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_${retrieve_suffix}${output_norm_suffix}"
