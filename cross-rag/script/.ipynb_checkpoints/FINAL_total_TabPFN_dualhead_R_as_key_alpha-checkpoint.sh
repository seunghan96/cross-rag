#!/bin/bash
# -*- coding: utf-8 -*-
# Total experiment script (TabPFN dualhead_R_as_key variant): Zero-shot with alpha (train set percentage) for all combinations
...
alpha_list=(${ALPHA_LIST:-5 10 20 25 50 75})

total_combinations=$((${#lookback_lengths[@]} * ${#similarity_spaces[@]} * ${#top_ks[@]} * ${#backbones[@]} * ${#output_norms[@]} * ${#output_norm_modes[@]} * ${#dual_lambda_list[@]} * ${#alpha_list[@]}))
            ...
            for backbone in "${backbones[@]}"; do
                    for output_norm in "${output_norms[@]}"; do
                        for output_norm_mode in "${output_norm_modes[@]}"; do
                            ...
                            filename_base="zeroshot_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}_TabPFN_dualhead_R_as_key_l${dual_lambda}_alpha${alpha}"
