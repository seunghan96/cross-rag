#!/bin/bash
# -*- coding: utf-8 -*-
# Random2 experiment script (TabPFN dualhead_R_as_key variant): Pretrain & Zero-shot with random sampling
# Both pretrain and zero-shot use random retrieval (retrieve_X_norm_random.py)
...
dual_lambda_list=(${TABPFN_DUAL_LAMBDA_LIST:-0.6})
                ...
                for output_norm in "${output_norms[@]}"; do
                    for output_norm_mode in "${output_norm_modes[@]}"; do
                        for dual_lambda in "${dual_lambda_list[@]}"; do
                            ...
                            model_id_pretrain="pretrain_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}${output_norm_suffix}_TabPFN_dualhead_R_as_key_l${dual_lambda}_random2"
...
if [ "$pretrain_skipped" = true ]; then
    ...
    continue
                            ...
                            filename_base="zeroshot_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}_TabPFN_dualhead_R_as_key_l${dual_lambda}_random2"
                            ...
                            for dataset in $datasets; do
                                ...
                                model_id_zeroshot="${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_${retrieve_suffix}${output_norm_suffix}_TabPFN_dualhead_R_as_key_l${dual_lambda}_random2"
