...
export CHRONOS_TABPFN_VARIANT=dualhead_R_as_key_custom
...
#forecast_horizons=(${FORECAST_HORIZONS:-12 16 24 36 48 64})
forecast_horizons=(${FORECAST_HORIZONS:-12})
...
#dual_lambda_list=(${TABPFN_DUAL_LAMBDA_LIST:-0.6 0.7 0.8})
dual_lambda_list=(${TABPFN_DUAL_LAMBDA_LIST:-0.8})
                    ...
                    for output_norm in "${output_norms[@]}"; do
                        for output_norm_mode in "${output_norm_modes[@]}"; do
                            for dual_lambda in "${dual_lambda_list[@]}"; do
                                ...
                                filename_base="zeroshot_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}_TabPFN_dualhead_R_as_key_l${dual_lambda}"
                                ...
                                for dataset in $datasets; do
                                    ...
                                    model_id_zeroshot="${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_${retrieve_suffix}${output_norm_suffix}_TabPFN_dualhead_R_as_key_l${dual_lambda}"
