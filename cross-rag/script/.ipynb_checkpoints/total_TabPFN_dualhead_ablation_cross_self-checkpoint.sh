#!/bin/bash
# -*- coding: utf-8 -*-
# Total experiment script (TabPFN dualhead_ablation_cross_self variant): Pretrain + Zero-shot for all combinations
...
export CHRONOS_TABPFN_VARIANT=dualhead_ablation_cross_self
...
output_norms=(false)
output_norm_modes=("x")
# hyperparameter: dualhead_ablation_cross_self mix lambda init
dual_lambda_list=(${TABPFN_DUAL_LAMBDA_LIST:-0.5})
                ...
                for output_norm in "${output_norms[@]}"; do
                    for output_norm_mode in "${output_norm_modes[@]}"; do
                        for dual_lambda in "${dual_lambda_list[@]}"; do
                            ...
                            model_id_pretrain="pretrain_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}${output_norm_suffix}_TabPFN_dualhead_ablation_cross_self_l${dual_lambda}"
...
if [ "$pretrain_skipped" = true ]; then
    ...
    continue
                            ...
                            filename_base="zeroshot_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}_TabPFN_dualhead_ablation_cross_self_l${dual_lambda}"
                            ...
                            for dataset in $datasets; do
                                ...
                                model_id_zeroshot="${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_${retrieve_suffix}${output_norm_suffix}_TabPFN_dualhead_ablation_cross_self_l${dual_lambda}"
