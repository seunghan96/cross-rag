#!/bin/bash
# -*- coding: utf-8 -*-
# Total experiment script (TabPFN dualhead variant, customizable horizons): Pretrain + Zero-shot for all combinations
...
export CHRONOS_TABPFN_VARIANT=dualhead_custom
export WANDB_DISABLED="${WANDB_DISABLED:-true}"
export WANDB_MODE="${WANDB_MODE:-disabled}"

run_file_pretrain=pretrain_custom.py
run_file_zeroshot=zeroshot_custom.py
gpu_loc=0
batch_size=256
...
datasets="${DATASETS:-"exchange_rate ETTh1 ETTh2 ETTm1 ETTm2 weather"}"
...
# Horizons (context / prediction) can be overridden via env vars INPUT_HORIZONS and FORECAST_HORIZONS
input_horizons=(${INPUT_HORIZONS:-512})
forecast_horizons=(${FORECAST_HORIZONS:-16 32 48})
...
output_norms=(false)
output_norm_modes=("x")
# hyperparameter: dualhead mix lambda init
dual_lambda_list=(${TABPFN_DUAL_LAMBDA_LIST:-0.4 0.5 0.6})

total_combinations=$((${#input_horizons[@]} * ${#forecast_horizons[@]} * ${#lookback_lengths[@]} * ${#similarity_spaces[@]} * ${#top_ks[@]} * ${#backbones[@]} * ${#output_norms[@]} * ${#output_norm_modes[@]} * ${#dual_lambda_list[@]}))
...
for input_horizon in "${input_horizons[@]}"; do
for forecast_horizon in "${forecast_horizons[@]}"; do
    seq_len=$input_horizon
    pred_len=$forecast_horizon
    context_length=$input_horizon
    prediction_length=$forecast_horizon

    # Scope checkpoints/results by horizons
    checkpoint_root="./checkpoints_${context_length}_${prediction_length}"
    results_root="results_${seq_len}_${pred_len}"
                    ...
                    for output_norm in "${output_norms[@]}"; do
                        for output_norm_mode in "${output_norm_modes[@]}"; do
                            for dual_lambda in "${dual_lambda_list[@]}"; do
                                ...
                                echo "input_horizon (context_length): $context_length"
                                echo "forecast_horizon (prediction_length): $prediction_length"
                                ...
                                model_id_pretrain="pretrain_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}${output_norm_suffix}_TabPFN_dualhead_l${dual_lambda}"

                                pretrain_checkpoint_dir="${checkpoint_root}/${model_id_pretrain}"
                                ...
                                filename_base="zeroshot_${backbone}_lb${lookback_length}_${similarity_space}_k${top_k}_TabPFN_dualhead_l${dual_lambda}"
                                ...
                                for dataset in $datasets; do
                                    ...
                                    model_id_zeroshot="${dataset}_zeroshot_${seq_len}_pred_${pred_len}_${lookback_length}_retrieve_${pred_len}_${retrieve_suffix}${output_norm_suffix}_TabPFN_dualhead_l${dual_lambda}"

                                    result_file="${results_root}/forecast_evaluation/${filename}"
                                    result_exists=false
